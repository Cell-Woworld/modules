...
namespace Database
{
	bool SQLServer::CreateInsertStatement(Array<String>& statement, Array<String>& param_list)
	{
		...
		{
			LOG_E(TAG, "SQLServer::CreateInsertStatement() Execption: %s", e.what());
		...
		for (unsigned int i = 0; i < _quoted_table_list.size(); i++)
		{
			...
			if (_is_values_type)
			{
				...
				_table_name.erase(std::remove(_table_name.begin(), _table_name.end(), ']'), _table_name.end());
				_table_name = std::regex_replace(_table_name, std::regex(".dbo"), "");
