...
namespace Database
{
	bool SQLServer::CreateSelectStatement(Array<String>& statement, Array<String>& param_list, bool create_stored_procedure)
	{
		...
		if (create_stored_procedure == false && _query_items[0] == "*")
		{
		...
		if (_cond_list.size() > 0)
		{
			...
			for (auto _cond = _cond_list.begin(); _cond != _cond_list.end(); ++_cond)
			{
				...
				if (_itr != param_list.end())
				{
					...
					_cond = _cond_list.insert(_cond, "get_list(");
					_cond = _cond_list.insert(_cond + 2, ", ' AND ')");
				}
			}
