...
namespace Database
{
	...
	{
		return new MySQL(owner);
	}
#endif

	Map<String, ProcedureInfo> MySQL::proccall_info_map_;
	Mutex MySQL::proccall_info_mutex_;
	...
	const String MySQL::MAX_VCHAR_PARAM_SIZE = "2048";
	const String MySQL::MAX_BINARY_PARAM_SIZE = "255";
	const String MySQL::MAX_MODEL_NAME_SIZE = "128";
	const Set<String> MySQL::_RESERVED_KEYWORD = { "","current_time" };
	const String MySQL::PROC_KEYWORDS[] = { "Database.Internal.Statement",
	...
	const String MySQL::CLAUSE_KEYWORDS[] = { "Database.Internal.Statement",
		"Database.Internal.Columns","Database.Internal.TableList","Database.Internal.ConditionList","Database.Internal.GroupBy",
		"Database.Internal.OrderBy","Database.Internal.FilterList"/*,"Database.Internal.ParameterList"*/,"Database.Internal.ModifierList",
		"Database.Internal.QueryClause","Database.Internal.Conjunction","Database.Internal.LastDepth","Database.Internal.LastState",
		"Database.Internal.ProcType","Database.Internal.CommitContent"
	};
	const Map<String, String> MySQL::SQL_KEYWORD =
	{
		{"SELECT ",		"Select "},
		{" FROM ",		" From "},
		{" WHERE ",		" Where "}
	};
	const Set<String> MySQL::NON_QUOTED_KEYWORD =
	{
		"null",
		"true",
		"false",
	...
	const String MySQL::SQL_SYMBOLS = "()%+-*/:'\", ;$";		// ; for internal use
	Obj<ObjectPool<ODBCAPI>> MySQL::db_connection_pool_ = nullptr;
	TimerQueue MySQL::timer_queue_;

	MySQL::MySQL(BioSys::IBiomolecule* owner)
		:RNA(owner, "MySQL", this)
		, owner_(owner)
	{
	...
	void MySQL::OnEvent(const DynaArray& name)
	{
		USING_BIO_NAMESPACE
		...
		case "Database.Task.Done"_hash:
		case "Database.Error"_hash:
		{
			NextCall();
			break;
		}
	...
	void MySQL::doCreateStoredProc()
	{
		...
		String _database_name = ReadValue<String>("Database.Internal.Name");
		...
		//String _output = "CREATE PROCEDURE " + _name + "(";
		String _output = "CREATE PROCEDURE " + _proc_full_name + "(";
		switch (_proc_type)
		{
		case ConstAPI::QUERY:
		{
			...
			for (unsigned int i = 0; i < _param_list.size(); i++)
			{
				...
				case BINARY:
					...
					break;
				default:
					//_type_str = " VARCHAR(" + MAX_VCHAR_PARAM_SIZE + ")";
					_type_str = " TEXT";
					break;
				}
			...
			{
				{
					...
					_db->execute("DROP PROCEDURE IF EXISTS " + _proc_full_name + ";");
	...
	void MySQL::doCallStoredProc()
	{
		...
		String _statement = "CALL " + _proc_name + "(";
		try
		{
				...
				try
				{
					...
					for (i = 0; i < _param_list.size(); i++)
					{
						...
						{	// function type
							...
							if (i < _param_list.size() - 1)
								_statement += _quote_string + RemoveAlias(_data_in_model) + _quote_string + ", ";
							else
								_statement += _quote_string + RemoveAlias(_data_in_model) + _quote_string;
						}
						else
						{
							...
							if (i < _param_list.size() - 1)
								_statement += _quote_string + (_perfrom_dynamic_condition && !_is_dynamic_condition_item && !_is_keyword ? "\\'" : "") + _data_in_model + (_perfrom_dynamic_condition && !_is_dynamic_condition_item && !_is_keyword ? "\\'" : "") + _quote_string + ", ";
							else
								_statement += _quote_string + (_perfrom_dynamic_condition && !_is_dynamic_condition_item && !_is_keyword ? "\\'" : "") + _data_in_model + (_perfrom_dynamic_condition && !_is_dynamic_condition_item && !_is_keyword ? "\\'" : "") + _quote_string;
						}
					}
	...
	bool MySQL::IsParameterFromModel(const String& parameter)
	{
		return (parameter[0] == ':' && parameter[1] == ':');
	}

	String MySQL::GetParameter(const String& parameter)
	{
		String _ret = std::move(rtrim(ltrim(parameter)));
		bool _is_model_name = IsParameterFromModel(_ret);
		if (_is_model_name)
		{
			_ret.assign(_ret.begin() + 2, _ret.end());
			Array<String> _param_list = ReadValue<Array<String>>("Database.Internal.ParameterList");
	...
	void MySQL::Done(const String& name, const String& alt_name)
	{
	...
	void MySQL::ReportError(const String& name, Error::ERROR_CODE code, const String& message, const String& alt_name)
	{
		...
		LOG_E(TAG, "MySQL::ReportError() code: %s, message: %s", _error_code_name.c_str(), message.c_str());
		if (alt_name != "")
			//RaiseEvent(alt_name);
			SendEvent(alt_name);
	...
	String MySQL::GetAlias(const String& var)
	{
	...
	String MySQL::RemoveAlias(const String& var)
	{
		size_t _last_alias = var.rfind(" AS ");
		if (_last_alias != String::npos)
	...
	String MySQL::RemoveAllAlias(const String& var)
	{
		String _var = var;
	...
	void MySQL::ResetAll(bool create_stored_procedure)
	{
	...
	void MySQL::ConvertToDynamicFormat(Array<String>& statement, Array<String>& param_list)
	{
	...
	void MySQL::ModifyTableName(Array<String>& table_list)
	{
		for (auto& _table_name : table_list)
		{
			if (_table_name.find_first_of(SQL_SYMBOLS) != String::npos)
				continue;
			if (_table_name.size() >= 2 && _table_name.front() == '`' && _table_name.back() == '`')
				continue;
			size_t _start_pos = _table_name.find('.');
			if (_start_pos == String::npos)
				continue;
			_table_name = "`" + _table_name.substr(0, _start_pos) + "`.`" + _table_name.substr(_start_pos + 1) + "`";
		}
	}
	void MySQL::ModifyColumnName(Array<String>& column_list)
	{
		for (auto& _column_name : column_list)
		{
			...
			while (_end_pos != String::npos)
			{
				...
				while (_end_pos != String::npos)
				{
					...
					{	// must be the Database.Internal.Name
						_start_pos++;
						if (_column_name.substr(_start_pos, _end_pos - _start_pos) == ReadValue<String>("Database.Internal.Name"))
							break;
					}
				}
			}
	...
	void MySQL::doFunction()
	{
		...
		if (!_function_alias.empty())
			_alias_target = ReadValue<String>("Database.Internal.Name") + "." + ReadValue<String>("Database.Internal.Title") + "." + _function_alias;
		//if (_target_model == "")
		//	_target_model = ReadValue<String>("Database.Internal.Name") + "." + ReadValue<String>("Database.Internal.Title") + "." + _function_alias;
		...
		case Function::DATE_FORMAT:
		{
				...
				if (i == 1)		// format of date-time
				{
					...
					for (unsigned int i = 0; i < _formats.size(); i++)
					{
						...
						if (itr != format_map::datetime.end())
							_parameter = std::regex_replace(_parameter, std::regex(itr->first), itr->second[format_map::MYSQL]);
					}
				}
			...
			if (_params != "")
			{
				...
				//String _result = _function_name + "(" + _params + ")";
				LOG_D(TAG, "MySQL::doFunction() writing result \"%s\" to model \"%s\"", _result.c_str(), _target_model != "" ? _target_model.c_str() : _alias_target.c_str());
			...
			{
				LOG_W(TAG, "MySQL::doFunction() Invalid parameters in function \"%s\"", _function_name.c_str());
			}
			break;
		}
		case Function::UTC_NOW:
		{
			_function_name = "UTC_TIMESTAMP";
			for (int i = 0; i < _function_parameters.size(); i++)
			{
		...
		case Function::TO_UNIXTIME: {
			_function_name = "UNIX_TIMESTAMP(CONVERT_TZ";
			for (int i = 0; i < _function_parameters.size(); i++)
			{
			...
			//if (_params != "")
			{
				String _result = _function_name + "(" + _params + ",'+00:00','SYSTEM'))";
				if (!_function_alias.empty())
					_result += " AS " + _function_alias;
				LOG_D(TAG, "MySQL::doFunction() writing result \"%s\" to model \"%s\"", _result.c_str(), _target_model != "" ? _target_model.c_str() : _alias_target.c_str());
				if (!_alias_target.empty())
		...
		case Function::FROM_UNIXTIME: {
			_function_name = "CONVERT_TZ(FROM_UNIXTIME";
			for (int i = 0; i < _function_parameters.size(); i++)
			{
				_params += RemoveAlias(GetParameter(_function_parameters[i]));
				if (i < _function_parameters.size() - 1)
			...
			//if (_params != "")
			{
				String _result = _function_name + "(" + _params + "),'SYSTEM','+00:00')";
				if (!_function_alias.empty())
					_result += " AS " + _function_alias;
				LOG_D(TAG, "MySQL::doFunction() writing result \"%s\" to model \"%s\"", _result.c_str(), _target_model != "" ? _target_model.c_str() : _alias_target.c_str());
				if (!_alias_target.empty())
		...
		case Function::FIND_IN_SET:
		case Function::ROUND:
		case Function::IFNULL:
		case Function::RAND:
		case Function::SHA2:
		case Function::TO_BASE64:
		case Function::FROM_BASE64:
		{
			for (int i = 0; i < _function_parameters.size(); i++)
			{
				_params += RemoveAlias(GetParameter(_function_parameters[i]));
		...
		case Function::BIN_TO_UUID:
		{
			...
			if (_params != "")
			{
				String _result = _function_name + "(" + _params + "; 1)";
				if (!_function_alias.empty())
					_result += " AS " + _function_alias;
				LOG_D(TAG, "MySQL::doFunction() writing result \"%s\" to model \"%s\"", _result.c_str(), _target_model != "" ? _target_model.c_str() : _alias_target.c_str());
				if (!_alias_target.empty())
		...
		case Function::CALC:
			if (_function_parameters.size() >= 1)
			{
				...
				if ((_start_pos = _param.find("::", _start_pos)) != String::npos)
				{
					...
					{
						_params += _param.substr(_end_pos, _start_pos - _end_pos);
						_end_pos = _param.find_first_of(SQL_SYMBOLS, _start_pos + 2);
						if (_end_pos != String::npos)
						{
							_params += RemoveAllAlias(GetParameter(_param.substr(_start_pos, _end_pos - _start_pos)));
						}
						else
						{
							_params += RemoveAllAlias(GetParameter(_param.substr(_start_pos)));
						}
						_start_pos = _param.find("::", _start_pos+2);
					}
					if (_end_pos != String::npos)
						_params += RemoveAllAlias(_param.substr(_end_pos));
				}
				else
				{
					_params = RemoveAllAlias(_param);
				}
				String _result = _params;
				if (!_function_alias.empty())
					_result += " AS " + _function_alias;
				LOG_D(TAG, "MySQL::doFunction() writing result \"%s\" to model \"%s\"", _result.c_str(), _target_model != "" ? _target_model.c_str() : _alias_target.c_str());
				if (!_alias_target.empty())
		...
		case Function::JSON_TABLE:
		{
			...
			{
				LOG_E(TAG, "MySQL::doFunction() Invalid parameter count for JSON_TABLE, it must be 2. Parameters must be splitted by ','.");
		...
		if (!_target_model.empty())
		{
			Remove("Database.Function.target_model_name");
		}
	}
	...
	void MySQL::doSetup()
	{
		...
		LOG_D(TAG, "##### MySQL::doSetup(%s)####", database_name_.c_str());
		String _connect_string = string_format(
			"Driver={%s};"					//MySQL ODBC #.# ANSI Driver;  // Don't use Unicode Driver
			"Server=%s; Port=%s;"
			"Database=%s;"
			"User=%s;Password=%s;"
			"CharSet=utf8;",
		...
		WriteValue("Database.Internal.Title", "");
		...
		std::function<ODBCAPI * ()> factory = [&]()
		{
			...
			{
				LOG_E(TAG, "MySQL::doSetup() Execption: %s", e.what());
				ReportError("Setup", Error::DATABASE_NOT_FOUND, "database name:" + ReadValue<String>("Database.Setup.database"));
			}
			return _db;
		};

		if (ReadValue<String>("Database.Internal.Name") == "")
			Clone("Database.Internal.Name", "Database.Setup.database");
		String _title = ReadValue<String>("Bio.Cell.Current.FullNamespace");
		Array<String> _ns_list;
		Split(_title, _ns_list, ".");
		if (_ns_list.size() >= 2)
			WriteValue("Database.Internal.Title", _ns_list[_ns_list.size() - 2]);
		else
			WriteValue("Database.Internal.Title", _ns_list.back());
	...
	void MySQL::doCreateClause()
	{
		...
		String _alias_target = ReadValue<String>("Database.Internal.Name") + "." + ReadValue<String>("Database.Internal.Title") + "." + _clause_alias;
		Array<String> _statement;
		Array<String> _param_list = ReadValue<Array<String>>("Database.Internal.ParameterList");
		switch (_proc_type)
		{
		...
		default:
			assert(false);
			ReportError(_alias_target, Error::CREATE_PROC_ERROR, "Unknown procedure type: '" + std::to_string(_proc_type) + "'", ReadValue<String>("Database.CreateClause.err_message"));
			return;
		}
		...
		{
			LOG_I(TAG, "MySQL::doCreateClause() statement: %s", _output.c_str());
		}
		WriteValue(_alias_target, "(" + _output + ") AS " + _clause_alias);
		if (!_target_model.empty())
			Clone(_target_model, _alias_target);
		...
		if (_name == "")
			_name = ReadValue<String>("Database.Internal.Title");
		String _proc_full_name = ReadValue<String>("Database.Internal.Name") + "." + _name;
		{
			MutexLocker proccall_info_lock_(proccall_info_mutex_);
			auto _itr = proccall_info_map_.find(_proc_full_name);
			if (_itr == proccall_info_map_.end())
		...
		if (!_target_model.empty())
		{
			...
			Done(_alias_target, ReadValue<String>("Database.CreateClause.done_message"));
	}

	void MySQL::KeepAlive(uint64_t interval, unsigned int pool_size)
	{
	...
	bool MySQL::is_datetime(const String& data)
	{
		//####-##-##T##:##:##.######
		bool _ret = true;
	...
	void MySQL::NextCall()
	{
		LOG_I(TAG, "[%p] NextCall() queue size=%zd", this, procedure_call_queue_.size());
		if (!procedure_call_queue_.empty())
		{
			RaiseEvent(procedure_call_queue_.front().event_name, procedure_call_queue_.front().event_content, (BioSys::IBiomolecule*)procedure_call_queue_.front().src_instance);
			LOG_I(TAG, "MySQL::NextCall() send event:%s", procedure_call_queue_.front().event_name.c_str());
			procedure_call_queue_.pop();
		}
		else
		{
			// It must be here. No time lag between queue checking and gateOpen
			//WriteValue("Database.CallSP.gateOpen", true);
		}
	}

	String MySQL::GetColumnType(const Map<String, SchemaInfo>& column_list, const String& column_name)
	{
		String _data_type = "";
	...
	void MySQL::doExecute()
	{
		LOG_D(TAG, "[%p] doExecute()", this);
		...
		if (_full_script != "")
		{
			...
			case ConstAPI::QUERY:
			{
				...
				//		_statement.push_back(_quoted_query_items[i] + ", ");
				//	else
				//		_statement.push_back(_quoted_query_items[i]);
				//}
				//_statement.push_back(" FROM ");
				//_statement.push_back(";\r\n");
				//for (unsigned int i = 0; i < _statement.size(); i++)
				//	_output += _statement[i];
		...
		else
		{
			...
			default:
			{
				...
				try
				{
					if (db_connection_pool_ == nullptr)
					{
						ReportError(_proc_full_name, Error::DATABASE_NOT_SETUP_YET, "Please \"Setup\" database connection first", ReadValue<String>("Database.CallStoredProc.err_message"));
						return;
					}
					Obj<ODBCAPI> _db = db_connection_pool_->borrow();
					_db->execute(_output);
